...
class SolverVBD(SolverBase, CouplingInterface):
    ...
    **soft mode**. Joint hard/soft mode is initialized from the optional
    ...
    def _init_rigid_system(
        ...
        # Zero-length body buffers for static-shape contact kernels when rigid state is absent.
        self._empty_body_q = wp.empty(0, dtype=wp.transform, device=self.device)
        self._empty_body_qd = wp.empty(0, dtype=wp.spatial_vector, device=self.device)
        self._empty_body_com = wp.empty(0, dtype=wp.vec3, device=self.device)
    ...
    def update_contacts(self, contacts: Contacts, state: State | None = None) -> None:
        ...
        if self._last_dt is None:
            raise ValueError(
                "SolverVBD.update_contacts requires a completed solver step before contact forces are available."
            )
        ...
        (
            ...
            rigid_contact_count,
        ) = self.collect_rigid_contact_forces(
