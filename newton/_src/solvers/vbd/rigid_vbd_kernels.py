...
def compute_soft_contact_forces(
    ...
    if body_index >= 0:
        ...
        if body_q_prev:
            ...
            body_vel_world = (body_pos_world - body_pos_prev) / dt + wp.transform_vector(
                X_wb, contact_body_vel[contact_idx]
            )
        else:
            ...
            body_vel_world = (
                body_vel_world + wp.spatial_top(body_v_s) + wp.cross(wp.spatial_bottom(body_v_s), point_offset)
            )
    ...
    penetration_depth = -(
        wp.dot(normal, particle_pos - body_pos_world) - particle_radius[particle_index] - soft_contact_margin
    )
    if penetration_depth <= 0.0:
        contact_force[rigid_contact_count[0] + contact_idx] = wp.spatial_vector(wp.vec3(0.0), wp.vec3(0.0))
        return
